...
function applyI18n() {
  ...
  document.querySelectorAll('.nav[data-view]').forEach(btn => {
    const label = btn.querySelector('[data-i18n]')?.textContent?.trim();
    if (!label) return;
...
const setBusy = (buttons, busy, label = '') => {
  ...
  list.filter(Boolean).forEach(btn => {
    if (busy) {
      ...
      btn.disabled = true;
      const busyText = label || (state.locale === 'zh' ? '保存中...' : 'Saving...');
...
function setHeader(title, desc, createText = '', showCreate = true, showQuickCreate = false) {
  ...
  if (createMenu) createMenu.style.display = showQuickCreate ? '' : 'none';
}
function setHeaderForView() {
  if (state.view === 'dashboard') setHeader(t('page.dashboard'), t('page.dashboardDesc'), '', false, true);
  if (state.view === 'shorts') setHeader(t('page.shorts'), t('page.shortsDesc'), t('btn.newShort'), true, false);
  if (state.view === 'lives') setHeader(t('page.lives'), t('page.livesDesc'), t('btn.newLive'), true, false);
  if (state.view === 'settings') setHeader(t('page.settings'), t('page.settingsDesc'), '', false, false);
}
...
async function renderDashboard() {
  setHeader(t('page.dashboard'), t('page.dashboardDesc'), '', false, true);
