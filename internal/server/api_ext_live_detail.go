...
func (s *Server) apiExtLiveQRDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeJSON(w, 400, apiErr("bad_id", "bad id"))
		return
	}
	if tail == "review" {
		if !actor.IsAdmin() {
			writeJSON(w, http.StatusForbidden, apiErr("forbidden", "admin only"))
			return
		}
	...
	if !canAccessLive(actor, current) {
		writeJSON(w, http.StatusForbidden, apiErr("forbidden", "no permission"))
		return
	}
	switch {
