...
func (s *Server) extLivePayloadToModel(p extLiveQRPayload, allowGenerate bool) (*model.LiveQR, error) {
	...
	var err error
	if code == "" && allowGenerate { code, err = s.generateUniqueCode("live"); if err != nil { return nil, err } }
	if err := util.ValidateCode(code); err != nil { return nil, err }
	status := p.Status; if status == "" { status = "active" }
	if status != "active" && status != "disabled" { return nil, fmt.Errorf("status must be active or disabled") }
	strategy := p.RotationStrategy; if strategy == "" { strategy = "round_robin" }
	if strategy != "round_robin" && strategy != "random" && strategy != "least_used" { return nil, fmt.Errorf("bad rotation strategy") }
	if p.FallbackURL != "" { p.FallbackURL = util.CleanURL(p.FallbackURL); if err := validateHTTPURL(p.FallbackURL); err != nil { return nil, err } }
	qrStyle, qrFg, qrBg := normalizeQRPayload(p.QRStyle, p.QRForeground, p.QRBackground)
	return &model.LiveQR{Code: code, Title: strings.TrimSpace(p.Title), Description: p.Description, Status: status, RotationStrategy: strategy, GuideTitle: p.GuideTitle, GuideText: p.GuideText, FallbackURL: p.FallbackURL, QRStyle: qrStyle, QRForeground: qrFg, QRBackground: qrBg}, nil
}
...
func (s *Server) extBundlePayloadToModels(p extLiveQRBundlePayload, allowGenerate bool) (*model.LiveQR, []model.LiveQRItem, error) {
	...
	for _, raw := range p.Items {
		...
		if err != nil { return nil, nil, err }
		item.ID = raw.ID
		if item.ID > 0 { if seen[item.ID] { return nil, nil, fmt.Errorf("duplicate item id") }; seen[item.ID] = true }
