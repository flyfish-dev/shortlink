...
const I18N = {
  ...
  en: {
    ...
    'settings.account':'Admin account','settings.accountDesc':'After binding an email, Magic Link login is available. Browser one-click login is preserved.','settings.email':'Email','settings.name':'Name','settings.saveAccount':'Save account','settings.system':'System','settings.appName':'Site name','settings.baseUrl':'Public domain','settings.locale':'Default language','settings.autoLocale':'Auto match browser','settings.loginMode':'Login mode','settings.hybrid':'Magic Link + browser one-click','settings.magic':'Magic Link only','settings.oneClick':'Browser one-click only','settings.database':'Database','settings.smtp':'SMTP / Magic Link','settings.smtpEnabled':'Enable SMTP','settings.smtpHost':'SMTP host','settings.smtpPort':'Port','settings.smtpSecurity':'Security','settings.smtpUsername':'Username','settings.smtpPassword':'Password / app token','settings.smtpPasswordHint':'Leave empty to keep current','settings.smtpFrom':'From email','settings.smtpSet':'Password saved','settings.smtpUnset':'No password saved','settings.saveSettings':'Save settings',
...
const themeIcon = (isDark) => isDark
  ? '<svg viewBox="0 0 24 24" aria-hidden="true"><circle class="theme-icon-fill" cx="12" cy="12" r="4.4"/><path class="theme-icon-accent" d="M12 2.9v2.2M12 18.9v2.2M4.3 4.3l1.6 1.6M18.1 18.1l1.6 1.6M2.9 12h2.2M18.9 12h2.2M4.3 19.7l1.6-1.6M18.1 5.9l1.6-1.6"/></svg>'
  : '<svg viewBox="0 0 24 24" aria-hidden="true"><path class="theme-icon-fill" d="M19.3 14.4A7.4 7.4 0 0 1 9.6 4.7 8.2 8.2 0 1 0 19.3 14.4Z"/><path class="theme-icon-accent" d="M16.8 3.7l.5 1.5 1.5.5-1.5.5-.5 1.5-.5-1.5-1.5-.5 1.5-.5.5-1.5Z"/></svg>';
...
function applyTheme() {
  ...
  if (btn) {
    ...
    btn.setAttribute('aria-label', isDark ? (state.locale === 'zh' ? '切换到浅色模式' : 'Switch to light mode') : (state.locale === 'zh' ? '切换到深色模式' : 'Switch to dark mode'));
...
async function loadMe() {
  ...
  if (me.app_name) document.getElementById('brandName').textContent = me.app_name;
  applyI18n();
}
...
async function saveSettings() {
  ...
  try {
    ...
    document.getElementById('brandName').textContent = data.data.app_name || 'AI短链平台';
