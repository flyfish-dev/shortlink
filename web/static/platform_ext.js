(() => {
  ...
  async function renderUsers() {
    ...
    $('#content').innerHTML = `<div class="toolbar"><button class="primary" id="newUserBtn">${txt('新建用户','New user')}</button></div><div class="card table-card"><table><thead><tr><th>ID</th><th>${txt('邮箱','Email')}</th><th>${txt('名称','Name')}</th><th>${txt('角色','Role')}</th><th>${txt('状态','Status')}</th><th>${txt('操作','Actions')}</th></tr></thead><tbody>${data.map(u => `<tr><td>${u.id}</td><td>${esc(u.email || '-')}</td><td>${esc(u.name || '-')}</td><td><span class="badge">${esc(u.role)}</span></td><td><span class="badge">${esc(u.status)}</span></td><td><button class="ghost" data-edit-user="${u.id}">${txt('编辑','Edit')}</button></td></tr>`).join('') || `<tr><td colspan="6"><div class="empty">${txt('暂无用户','No users')}</div></td></tr>`}</tbody></table></div>`;
